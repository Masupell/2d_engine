...
// pub fn make_context<'a>(state: &'a mut State, input: &'a mut Input, dt: f64) -> EngineContext<'a> 
// {
//     EngineContext::new(&mut state.renderer, input, &mut state.device, &mut state.queue, &mut state.config).with_dt(dt)
// }
...
// impl<'a> EngineContext<'a> // Has everything right now, including the LoadingContex (which does not exist anymore, but I will re-add as Asset Manager)
// {
//     pub fn new(renderer: &'a mut Renderer, input: &'a Input, device: &'a wgpu::Device, queue: &'a wgpu::Queue, config: &'a wgpu::SurfaceConfiguration) -> Self
//     {
//         Self
//         {
//             renderer,
